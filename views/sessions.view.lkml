...
view: sessions {
      ...
      session_list_with_event_history as (
        select timestamp(PARSE_DATE('%Y%m%d', REGEXP_EXTRACT(_TABLE_SUFFIX,r'[0-9]+'))) session_date
            ...
            , ARRAY(select AS struct it.* EXCEPT(item_params) from unnest(events.items) as it) as items
              ...
              where {% incrementcondition %} timestamp(PARSE_DATE('%Y%m%d', REGEXP_EXTRACT(_TABLE_SUFFIX,r'[0-9]+'))) {%  endincrementcondition %} -- full
              -- where ( (timestamp(PARSE_DATE('%Y%m%d', REGEXP_EXTRACT(_TABLE_SUFFIX,r'[0-9]+'))) >= TIMESTAMP('2024-04-09 00:00:00'))
              -- or (timestamp(PARSE_DATE('%Y%m%d', REGEXP_EXTRACT(_TABLE_SUFFIX,r'[0-9]+'))) <= TIMESTAMP('2024-04-01 00:00:00')) )
              -- or (timestamp(PARSE_DATE('%Y%m%d', REGEXP_EXTRACT(_TABLE_SUFFIX,r'[0-9]+'))) = TIMESTAMP('2024-04-07 00:00:00'))
              -- and (timestamp(PARSE_DATE('%Y%m%d', REGEXP_EXTRACT(_TABLE_SUFFIX,r'[0-9]+'))) <= TIMESTAMP('2024-05-01 00:00:00'))--citla
  ...
  extends: [event_funnel, page_funnel, _date_comparison]

## Custom dimensions

  dimension: full_landing {
    type: string
    sql: CONCAT(${device_data_web_info_hostname},${landing_page}) ;;
  }

  dimension: landing_type {
    type: string
    sql:
    CASE
      WHEN REGEXP_CONTAINS(${full_landing}, r"^(www\.)?(leblancsparesorts|palaceresorts|moonpalace)\.com")
        THEN "Landing Brand - Contenido"

      WHEN REGEXP_CONTAINS(${full_landing}, r"^^onlinebookingspr\.(leblancsparesorts|palaceresorts|moonpalace)\.com")
      THEN "Landing Brand - Motor"

      WHEN (${full_landing} LIKE "%leblancsparesorts.com%"
      OR ${full_landing} LIKE "%palaceresorts.com%"
      OR ${full_landing} LIKE "%moonpalace.com%"
      OR ${full_landing} LIKE "%moonpalacecancun.com%")
      AND (${full_landing} NOT LIKE "package%"
      OR ${full_landing} NOT LIKE "paquetes%")
      THEN "Landing Propiedad"
      ELSE "Otros"
      END
      ;;
  }
  ...
  measure: salto_a_motor {
    type: number
    sql: ${sesiones_motor}/NULLIF(${sesiones_contenido},0) ;;
    value_format_name: percent_2
  }

  measure: sesiones_rooms {
    type: count_distinct
    sql: ${TABLE}.sl_key ;;
    filters: [events.event_param_page: "/rooms"]
    hidden: yes
  }

  measure: motor_conversion_rate {
    type: number
    sql: ${events.total_transactions}/NULLIF(${sesiones_rooms},0) ;;
    value_format_name: percent_2
  }
...
## Dimensions
  ...
  dimension: geo_data_country_indicator {
    view_label: "Audience"
    group_label: "User"
    label: "Bad Traffic"
    type:  string
    sql:CASE
      WHEN ${geo_data_country} = "France" AND ${TABLE}.session_date <= "2024-06-15"
      THEN "false"
      ELSE "legit"
      END ;;
  }
...
## Measures
  ...
  measure: percentage_returning_users_2 {
    view_label: "Audience"
    group_label: "User"
    label: "% Returning Users"
    type: number
